...
async function handleScheduled(params, service) {
  let time = params.get("time"), scheduledTime = time ? new Date(parseInt(time)) : void 0, cron = params.get("cron") ?? void 0, format = params.get("format"), result = await service.scheduled({
    scheduledTime,
    cron
  });
  return format === "json" ? Response.json(result, {
    status: result.outcome === "ok" ? 200 : 500
  }) : new Response(result.outcome, {
    status: result.outcome === "ok" ? 200 : 500
  });
}
...
function stringify_string(str) {
  ...
  for (let i2 = 0; i2 < len; i2 += 1) {
    let char = str[i2], replacement = get_escaped_char(char);
    replacement && (result += str.slice(last_pos, i2) + replacement, last_pos = i2 + 1);
  }
...
function valid_array_indices(array) {
  ...
  for (var i2 = keys.length - 1; i2 >= 0 && !is_valid_array_index(keys[i2]); i2--)
    ;
  return keys.length = i2 + 1, keys;
}
...
function asciiToBinary(data) {
  ...
  for (let i2 = 0; i2 < data.length; i2++)
    buffer <<= 6, buffer |= KEY_STRING.indexOf(data[i2]), accumulatedBits += 6, accumulatedBits === 24 && (output += String.fromCharCode((buffer & 16711680) >> 16), output += String.fromCharCode((buffer & 65280) >> 8), output += String.fromCharCode(buffer & 255), buffer = accumulatedBits = 0);
...
function binaryToAscii(str) {
  ...
  for (let i2 = 0; i2 < str.length; i2 += 3) {
    ...
    groupsOfSix[0] = str.charCodeAt(i2) >> 2, groupsOfSix[1] = (str.charCodeAt(i2) & 3) << 4, str.length > i2 + 1 && (groupsOfSix[1] |= str.charCodeAt(i2 + 1) >> 4, groupsOfSix[2] = (str.charCodeAt(i2 + 1) & 15) << 2), str.length > i2 + 2 && (groupsOfSix[2] |= str.charCodeAt(i2 + 2) >> 6, groupsOfSix[3] = str.charCodeAt(i2 + 2) & 63);
  ...
  function hydrate(index, standalone = !1) {
    ...
    if (!value || typeof value != "object")
      ...
      if (typeof value[0] == "string") {
        ...
        if (reviver) {
          let i2 = value[1];
          if (typeof i2 != "number" && (i2 = values.push(value[1]) - 1), hydrating ??= /* @__PURE__ */ new Set(), hydrating.has(i2))
            throw new Error("Invalid circular reference");
          return hydrating.add(i2), hydrated[index] = reviver(hydrate(i2)), hydrating.delete(i2), hydrated[index];
        }
        switch (type) {
          case "Date":
      ...
      switch (type) {
        ...
        case "Array": {
          ...
          for (let i2 = 0; i2 < thing.length; i2 += 1)
            if (i2 > 0 && (str += ","), Object.hasOwn(thing, i2))
              keys.push(`[${i2}]`), str += flatten(thing[i2]), keys.pop();
...
function validateCdnCgiRequest(request, routes, upstreamUrl) {
  if (!isCdnCgiRequest(request.url) && !isCdnCgiRequest(request.headers.get(CoreHeaders.ORIGINAL_URL)))
    return;
  let exactHostnames = /* @__PURE__ */ new Set(), wildcardHostnames = /* @__PURE__ */ new Set();
  for (let route of routes) {
    let hostname = route.hostname.replace(/^\./, "");
    hostname && (route.allowHostnamePrefix ? wildcardHostnames.add(hostname) : exactHostnames.add(hostname));
  }
  if (upstreamUrl)
    try {
      let upstreamHostname = new URL(upstreamUrl).hostname;
      upstreamHostname && exactHostnames.add(upstreamHostname);
    } catch {
    }
...
function isHostnameAllowed(hostname, exactHostnames, wildcardHostnames) {
  if (LOCALHOST_HOSTNAMES.includes(hostname) || exactHostnames.has(hostname))
    return !0;
  for (let allowed of wildcardHostnames)
...
var entry_worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      let imagesDelivery = env[CoreBindings.SERVICE_IMAGES_DELIVERY];
      if ((url.pathname === CorePaths.IMAGE_DELIVERY || url.pathname.startsWith(`${CorePaths.IMAGE_DELIVERY}/`)) && imagesDelivery)
      ...
      let r2PublicService = env[CoreBindings.SERVICE_R2_PUBLIC];
      if ((url.pathname === CorePaths.R2_PUBLIC || url.pathname.startsWith(`${CorePaths.R2_PUBLIC}/`)) && r2PublicService)
